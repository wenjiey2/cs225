...
class KDTree
{
  ...
  private:
     ...
     */
     KDTreeNode* buildTree(int start, int end, int d);
     void quickSelect(int start, int end , int d, int mid);
     int partition(int start, int end, int pIndex, int d);
     void clear(KDTreeNode* subroot);
     double getDistance(const Point<Dim> & p1, const Point<Dim> & p2) const;
     Point<Dim> findNearest(const Point<Dim>& query, KDTreeNode* subroot, int d) const;
};
