...
class BinaryTree
{
  ...
  private:
     ...
     */
     ...
     bool isOrderedR(const Node* root) const;

     bool isOrderedI(typename BinaryTree<T>::Node* root) const;

     void printP(vector<vector<T> > &paths, const Node* subRoot, vector<T> curr) const;

     int & sumDist(int dist, const Node* subRoot, int &sum) const;
